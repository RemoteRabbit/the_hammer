...
class All_Users(commands.Cog):
    ...
    async def joined(self, ctx, user: discord.Member):
        ...
        await ctx.send(content=f'<@{user_id}> ({user.name}#{user.discriminator}) joined in {user_joined.strftime("%d")} {refined_month} {user_joined.strftime("%Y")} at {user_joined.strftime("%H:%M")} UTC')
